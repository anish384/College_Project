...
// Download images for header logos
...
try {
    // Get faculty_id from GET parameters
    ...
    if ($faculty_info) {
        // Fixed starting row for faculty info
        $startRow = 13;
        $currentRow = $startRow;
    
        // Set column widths for image display area
        $sheet->getColumnDimension('G')->setWidth(20);
        $sheet->getColumnDimension('H')->setWidth(20);
    
        // Add faculty image if it exists in faculty_table
        if (!empty($faculty_info['image'])) {
            // Construct the absolute image path
            $baseDir = __DIR__;  // Directory of current script (Display/)
            $imagePath = $baseDir . '/' . $faculty_info['image'];  // e.g., img/1738908286_citations.jpg
            ...
            if (file_exists($imagePath)) {
                try {
                    ...
                    // Merge cells G13 to H19 as specified
                    $sheet->mergeCells('G13:H19');
                    // Place the image at the top left cell of the merged range
                    $drawing->setCoordinates('G13');
                    // Optionally, adjust the size as needed
        ...
        // Faculty Info Array - mapping labels to database fields
        $facultyDetails = [
        ...
        // Add faculty details to the sheet
    ...
    // List of additional tables for faculty's details
    $tables_info = [
