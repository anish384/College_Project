    ...
    try {
        ...
        if ($stmt->execute()) {
            echo "<script>
                ...
                window.location.href='test.php?faculty_id=$faculty_id';
            </script>";
        } else {
...
<main>
    ...
    foreach ($tables as $table) {
        ...
        if ($result->num_rows > 0) {
            echo "<table><tr>";
            echo "<th>Sr No</th>"; // Added Serial Number column
            
            $columns = array_keys($result->fetch_assoc());
            $filtered_columns = array_filter($columns, function($col) {
                return $col !== "faculty_id"; // Skipping faculty_id
            });
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>
                        <button class='edit-btn' onclick=\"toggleForm('edit_$table-{$row[$columns[0]]}')\">Edit</button>
                        <form id='edit_$table-{$row[$columns[0]]}' class='edit-form' method='POST' style='display:none;'>
                            <input type='hidden' name='table_name' value='$table'>
                            <input type='hidden' name='row_id' value='{$row[$columns[0]]}'>
                            <input type='hidden' name='field_name' value='{$filtered_columns[1]}'>
                            <input type='text' name='new_value' value='" . htmlspecialchars($row[$filtered_columns[1]]) . "' required>
        ...
        echo "<button class='add-btn' onclick=\"toggleForm('add_$table')\">Add New Entry</button>";
        echo "<form id='add_$table' class='add-form' method='POST' style='display:none;'>";
        ...
        while ($column = $result_columns->fetch_assoc()) {
            if ($column['Field'] !== "id" && $column['Field'] !== "faculty_id") { // Skipping ID & faculty_id
                echo "<label>" . ucfirst(str_replace("_", " ", $column['Field'])) . ":</label>";
                echo "<input type='text' name='data[" . $column['Field'] . "]' required>";
            }
        }

        echo "<button type='submit' name='add_entry' class='save-btn'>Submit</button>";
        echo "</form>";
